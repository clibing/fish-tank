...
static inline bool dht_fetch_data(dht_sensor_type_t sensor_type, uint8_t pin, bool bits[DHT_DATA_BITS]) {
    ...
    if (!dht_await_pin_state(pin, 40, false, NULL)) {
        ESP_LOGD(DHT_TAG, "Initialization error, problem in phase 'B'");
        return false;
    }

    // Step through Phase 'C', 88us
    if (!dht_await_pin_state(pin, 88, true, NULL)) {
        ESP_LOGD(DHT_TAG, "Initialization error, problem in phase 'C'\n");
        return false;
    }

    // Step through Phase 'D', 88us
    if (!dht_await_pin_state(pin, 88, false, NULL)) {
        ESP_LOGD(DHT_TAG, "Initialization error, problem in phase 'D'\n");
        return false;
    }
    ...
    for (int i = 0; i < DHT_DATA_BITS; i++) {
        if (!dht_await_pin_state(pin, 65, true, &low_duration)) {
            ESP_LOGD(DHT_TAG, "LOW bit timeout\n");
            return false;
        }
        if (!dht_await_pin_state(pin, 75, false, &high_duration)) {
            ESP_LOGD(DHT_TAG, "HIGHT bit timeout\n");
            return false;
        }
...
esp_err_t dht_read_data(dht_sensor_type_t sensor_type, gpio_num_t pin, int16_t *humidity, int16_t *temperature) {
    ...
    if (data[4] != ((data[0] + data[1] + data[2] + data[3]) & 0xFF)) {
        ESP_LOGD(DHT_TAG, "Checksum failed, invalid data received from sensor\n");
        return ESP_FAIL;
    }

    *humidity = dht_convert_data(sensor_type, data[0], data[1]);
    *temperature = dht_convert_data(sensor_type, data[2], data[3]);
    ESP_LOGD(DHT_TAG, "Sensor data: humidity=%d, temp=%d\n", *humidity, *temperature);
